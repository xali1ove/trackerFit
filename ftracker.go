...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	var dist float64 // Переименовали переменную
	var speed float64
	var calories float64

	switch trainingType {
	case "Бег":
		dist = distance(action) // Используем функцию distance
	...
	case "Плавание":
		dist = swimmingMeanSpeed(lengthPool, countPool, duration) * duration
